#!/usr/bin/env python3
# plan_manager_gui.py - 计划储存自动化程序(可视化版)
...
class PlanManagerGUI:
    ...
    def create_menu(self):
        ...
        view_menu.add_command(label="只显示未完成", command=lambda: self.filter_by_completion(False))
        view_menu.add_command(label="只显示已完成", command=lambda: self.filter_by_completion(True))
    ...
    def create_filter_widgets(self):
        """创建筛选面板组件"""
        ttk.Label(self.filter_frame, text="筛选", font=('Arial', 12, 'bold')).pack(pady=(0, 10), anchor=tk.W)
        ...
        for text, value in priorities:
            ttk.Radiobutton(priority_frame, text=text, value=value, 
        ...
        completions = [("全部", "all"), ("未完成", "uncompleted"), ("已完成", "completed")]
        for text, value in completions:
            ttk.Radiobutton(completion_frame, text=text, value=value, 
        ...
        ttk.Button(self.filter_frame, text="应用标签筛选", 
                  command=self.apply_tag_filter).pack(pady=5, anchor=tk.W)
        
        # 分隔线
        ttk.Separator(self.filter_frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)
        
        # 清除所有筛选按钮
        ttk.Button(self.filter_frame, text="清除所有筛选", 
                  command=self.clear_filters).pack(pady=10, anchor=tk.W)
        
        # 即将到期筛选
        ttk.Button(self.filter_frame, text="查看即将到期", 
                  command=self.show_upcoming).pack(pady=5, anchor=tk.W)
    ...
    def create_plan_list(self):
        ...
        ttk.Label(header_frame, text="计划列表", font=('Arial', 14, 'bold')).pack(side=tk.LEFT)
        
        # 计划列表（使用Treeview）
        columns = ("id", "title", "priority", "deadline", "tags", "status")
        self.plan_tree = ttk.Treeview(self.content_frame, columns=columns, show="headings", 
                                     style="Treeview")
        ...
        # 添加滚动条
        scrollbar = ttk.Scrollbar(self.content_frame, orient=tk.VERTICAL, command=self.plan_tree.yview)
        ...
        self.context_menu.add_command(label="删除计划", command=self.delete_selected_plan)
        self.context_menu.add_separator()
        self.context_menu.add_command(label="标记为已完成", command=self.complete_selected_plan)
    ...
    def create_action_buttons(self):
        ...
        ttk.Button(button_frame, text="添加计划", style="Accent.TButton",
                  command=self.show_add_plan_dialog).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="编辑计划", 
                  command=self.edit_selected_plan).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="删除计划", 
                  command=self.delete_selected_plan).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="标记完成", 
                  command=self.complete_selected_plan).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="查看详情", 
                  command=self.view_plan_details).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="刷新", 
                  command=self.load_plans).pack(side=tk.RIGHT, padx=5)
    ...
    def load_plans(self):
        ...
        for plan in plans:
            ...
            priority = priority_map.get(plan["priority"], plan["priority"])
            ...
            item_id = self.plan_tree.insert("", tk.END, values=(plan_id, title, priority, deadline, tags, status), tags=(tag,))
        ...
        self.plan_tree.tag_configure("completed", background="#e0e0e0", foreground="#888888")
    ...
    def show_upcoming(self):
        """显示即将到期的计划"""
        days = simpledialog.askinteger("即将到期", "请输入天数:", initialvalue=7, minvalue=1, maxvalue=365)
        if days is None:
            return
        ...
        # 显示过滤提示
        if plans:
            messagebox.showinfo("即将到期", f"显示未来 {days} 天内即将到期的 {len(plans)} 个计划")
        else:
    ...
    def view_plan_details(self):
        ...
        # 获取计划详情
        plans = self.plan_manager.plans["plans"]
        ...
        # 标题
        ttk.Label(main_frame, text=plan["title"], font=('Arial', 16, 'bold')).pack(anchor=tk.W, pady=(0, 10))
        ...
        priority_value = tk.Label(priority_frame, text=plan["priority"].upper(), 
                                 bg=priority_colors.get(plan["priority"], self.bg_color),
                                 font=('Arial', 10, 'bold'))
        ...
        desc_text.insert(tk.END, plan["description"])
        ...
        if plan["deadline"]:
            ttk.Label(left_frame, text=f"截止日期: {plan['deadline']}", font=('Arial', 10)).pack(anchor=tk.W, pady=2)
        else:
            ttk.Label(left_frame, text="截止日期: 无", font=('Arial', 10)).pack(anchor=tk.W, pady=2)
            
        ttk.Label(left_frame, text=f"创建于: {plan['created_at']}", font=('Arial', 10)).pack(anchor=tk.W, pady=2)
        
        # 右侧信息
        right_frame = ttk.Frame(info_frame)
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        
        status_text = "已完成" if plan["completed"] else "未完成"
        ttk.Label(right_frame, text=f"状态: {status_text}", font=('Arial', 10)).pack(anchor=tk.W, pady=2)
        
        tags_text = ", ".join(plan["tags"]) if plan["tags"] else "无"
        ttk.Label(right_frame, text=f"标签: {tags_text}", font=('Arial', 10)).pack(anchor=tk.W, pady=2)
        
        # 底部按钮
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill=tk.X, pady=(10, 0))
        
        ttk.Button(button_frame, text="编辑", command=lambda: [details_window.destroy(), self.edit_selected_plan()]).pack(side=tk.LEFT, padx=5)
        
        if not plan["completed"]:
            ttk.Button(button_frame, text="标记为已完成", 
                      command=lambda: [details_window.destroy(), self.complete_plan(plan_id)]).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="关闭", command=details_window.destroy).pack(side=tk.RIGHT, padx=5)
    ...
    def show_plan_dialog(self, plan_id=None):
        ...
        dialog.title("添加计划" if not plan else "编辑计划")
        ...
        title_var = tk.StringVar(value=plan["title"] if plan else "")
        ...
        # 截止日期
        ttk.Label(main_frame, text="截止日期 (YYYY-MM-DD):").pack(anchor=tk.W, pady=(0, 5))
        deadline_var = tk.StringVar(value=plan["deadline"] if plan and plan["deadline"] else "")
        ...
        priority_var = tk.StringVar(value=plan["priority"] if plan else "medium")
        priority_frame = ttk.Frame(main_frame)
        ...
        for text, value in priorities:
            ttk.Radiobutton(priority_frame, text=text, value=value, 
                           variable=priority_var).pack(side=tk.LEFT, padx=10)
        
        # 标签
        ttk.Label(main_frame, text="标签 (用逗号分隔):").pack(anchor=tk.W, pady=(0, 5))
        tags_var = tk.StringVar(value=", ".join(plan["tags"]) if plan and plan["tags"] else "")
        tags_entry = ttk.Entry(main_frame, textvariable=tags_var)
        tags_entry.pack(fill=tk.X, pady=(0, 10))
        
        # 完成状态
        status_var = tk.BooleanVar(value=plan["completed"] if plan else False)
        ...
        ttk.Button(button_frame, text="保存", command=save_plan, style="Accent.TButton").pack(side=tk.RIGHT, padx=5)
    ...
    def delete_selected_plan(self):
        ...
        # 确认删除
        confirm = messagebox.askyesno("确认删除", "确定要删除这个计划吗？此操作不可恢复。")
        if not confirm:
            return
    ...
    def export_data(self):
        """导出计划数据"""
        messagebox.showinfo("导出数据", f"数据已保存到 {self.plan_manager.storage_path}")
