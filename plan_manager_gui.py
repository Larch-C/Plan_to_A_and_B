...
class PlanManagerGUI:
    ...
    def view_plan_details(self):
        ...
        # 标题
        ttk.Label(main_frame, text=plan["title"], font=("Arial", 16, "bold")).pack(
        ...
        if plan["deadline"]:
            ttk.Label(
                left_frame, text=f"截止日期: {plan['deadline']}", font=("Arial", 10)
        ...
        ttk.Label(
            left_frame, text=f"创建于: {plan['created_at']}", font=("Arial", 10)
        ).pack(anchor=tk.W, pady=2)
    ...
    def show_plan_dialog(self, plan_id=None):
        ...
        deadline_var = tk.StringVar(
            value=plan["deadline"] if plan and plan["deadline"] else ""
        )
        ...
        tags_var = tk.StringVar(
            value=", ".join(plan["tags"]) if plan and plan["tags"] else ""
        )
